...
def multi_hash(img):
    """计算图像的多种哈希值"""
    return (imagehash.phash(img), imagehash.dhash(img), imagehash.average_hash(img))
...
def create_font_mapping(
    enc_font_path: Path, std_font_path: Path, output_json: Path
) -> dict[str, str]:
    ...
    if output_json:
        write_json(output_json, mapping)
    return mapping
