    ...
    def __init__(
        ...
        if visualization_type == "spectrum":
            ...
            self.bars = self.ax.bar(
                range(len(self.bars_data)), self.bars_data, color="cyan", width=0.8
            )
    ...
    def update_plot(self, frame):
        ...
        try:
            ...
            if self.visualization_type == "spectrum":
                ...
                spectrum_grouped = np.array(
                    [
                        np.mean(spectrum[i : i + len(spectrum) // n_bars])
                        for i in range(0, len(spectrum), len(spectrum) // n_bars)
                    ]
                )[:n_bars]
                ...
                # 更新条形高度和颜色
                for bar, h in zip(self.bars, self.bars_data, strict=False):
                    bar.set_height(h)
                    # 根据高度设置颜色
