...
def setup_logging() -> None:
    ...
    log_path: Path = utils.writable_path("data", "log", "py", f"python_{timestamp}.log")
...
def ensure_files(
    path_dict: dict[str, list[str]], path_method: Callable[..., Path], task: Callable[[Path], None]
) -> None:
    """载入必要路径并确保所有路径存在"""
    path: list[str]
...
if __name__ == "__main__":
    ...
    application.setWindowIcon(
        QIcon(str(utils.static_path("src", "resources", "ico", "the_icon.ico")))
    )
    ...
    engine.load(QUrl.fromLocalFile(utils.static_path("src", "gui", "main.qml")))
