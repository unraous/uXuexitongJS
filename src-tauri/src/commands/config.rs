...
#[tauri::command]
#[specta::specta]
pub fn refresh_ollama_models() -> CommandsResult<Vec<String>> {
    log::debug!("正在刷新 Ollama 模型列表...");
    let mut providers = CONFIG.llm.providers.lock();
    let p = providers
        .get_mut("ollama")
        .ok_or_else(|| anyhow::anyhow!("找不到 Ollama 提供商"))?;

    let models = crate::config::llm::ollama::fetch_models(&p.base_url);
    if !models.is_empty() {
        p.models = models;
        p.chosen_model = Some(0);
    }
    log::info!("Ollama 模型列表刷新完成: {:?}", p.models);
    Ok(p.models.clone())
}
